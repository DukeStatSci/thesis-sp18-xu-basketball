...
kable(mat) %>% kable_styling(latex_options = ("scale_down")) %>% kable_styling(font_size = 6)
```
...
##Additive and Multiplicative Effects Network (AMEN) Analysis
...
$$y_{ij} =  \beta_{d}x_{d} + r_{i} + s_{j} + u_{i}^{T}v_{j} + \epsilon_{ij}$$ where $$r_{i} = \beta_{i}x_{i} + a_{i}$$ and $$s_{j} = \beta_{j}x_{j} + b_{j}$$
The model captures the network structure of each pass possession by transforming the passing networks into adjacency matrices. Row and column nodal covariates are px1xn vectors indicating the contribution of each player p for each possession n. The dyadic features are pxpxnxi arrays that capture the shared features between players.  
