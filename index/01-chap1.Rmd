...
Previous work conducted by Daniel Cervone, Alex, D'Amour, Luke Bornn, and Kirk Goldsberry attempt to capture the game wholelistically via a new measure called Expected Possession Value (EPV) and Expected Possession Value -Adjusted (EPVA). This new metric uses three models--a microtransition model, macrotransition entrance model, and a macrotransition exit model--to capture the spatial biases of each player and the in-game effects of pressure. 

##Microtransition Model
$equation$
The microtransition model models the defensive conditions of the game . The macro model has two parts, depending on the the given transition states. The first part is a binomial .... the second a poisson... This model performs well, although the computation time prevents any real-time analyses of the game. 
...
A simpler model may perform more quickly and potentially just as robustly.
